...
def gen_splits(args, phenos):
    ...
    if args.task == 'token':
        ...
        raise ValueError
# if files are raw data, subjects should be the names of all files 
...
class MyDataset(Dataset):
    def __init__(self, args, tokenizer, data_source, phenos, train = False):
        ...
        self.meddec_stats = pd.read_csv(os.path.join(args.data_dir, 'stats.csv')).set_index(['SUBJECT_ID', 'HADM_ID'])
    ...
    def load_phenos(self, args, row, idx):
        txt_path = os.path.join(args.data_dir, f'raw_text/{row["subject_id"]}_{row["hadm_id"]}.txt') #_{row["row_id"]}
    ...
    def load_decisions(self, args, fn, idx, phenos):
        # fn 是某个training file-医生的annotation label 的地址： 'data/11216_188044_9756.json'
        basename = os.path.splitext(os.path.basename(fn))[0]

        file_dir = os.path.join(args.data_dir, fn)  

        sid, hadm= map(int, basename.split('_')[:2])
        ...
        if (sid, hadm) in phenos.index:
            sample_phenos = phenos.loc[sid, hadm]['phenotype_label']
            if isinstance(sample_phenos, pd.Series):
                sample_phenos = ','.join(sample_phenos.astype(str))
            for pheno in sample_phenos.split(','):
        ...
        row = self.meddec_stats.loc[sid, hadm]
...
def load_data(args):
    ...
    phenos.set_index(['subject_id', 'hadm_id'], inplace=True)
################above ok###################
